...
export const CalendarEvent: React.FC<CalendarEventProps> = ({
  ...
  return <div onClick={handleClick} className="bg-white/70 flex w-full items-center gap-4 mt-2.5 p-4 cursor-pointer hover:bg-white/80 transition-colors rounded-2xl">
      {showWorkIcon && !isTransaction && <div className="bg-[rgba(0,0,0,0.03)] self-stretch flex items-center justify-center gap-2.5 w-10 h-10 my-auto p-2.5 rounded-[500px]">
          <Briefcase className="w-5 h-5 text-gray-600" />
        </div>}
      {isTransaction && <div className="bg-[#FEF7CD] self-stretch flex items-center justify-center gap-2.5 w-10 h-10 my-auto p-2.5 rounded-[500px]">
          <DollarSign className="w-5 h-5 text-black" />
        </div>}
      {icon && !showWorkIcon && !isTransaction && <div className="bg-[rgba(0,0,0,0.03)] self-stretch flex items-center gap-2.5 w-10 h-10 my-auto p-2.5 rounded-[500px]">
      ...
      <div className="self-stretch text-xs text-gray-500 font-normal leading-loose flex-1 shrink basis-5 my-auto">
        ...
        {start_time && end_time && <div>{formatTime(start_time)} - {formatTime(end_time)}</div>}
        {location && <div>{location}</div>}
        {withPeople && withPeople.length > 0 && <div className="flex w-full items-center">
      ...
      {displayAmount !== undefined && <div className={`text-sm ${getAmountClass()} font-medium`}>
